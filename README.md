...
[UV](https://docs.astral.sh/uv/getting-started/installation/) must be installed on your machine.
...
```bash
mkdir -p output
uv run python -u src/baseline.py | tee output/baseline.txt
```

Run with router and save the output to a file.

```bash
uv run python -u src/with_router.py | tee output/with_router.txt
```

Compare the results.

```bash
uv run python -u src/compare.py | tee output/comparison.txt
```
